...
public class TileFixedStandRenderer extends TileEntitySpecialRenderer<TileEntityBrewingStand> {
    private void renderPotion(int slot, ItemStack slotStack) {
        ...
        Minecraft.getMinecraft().getRenderManager().renderEntity(slotEntity, 0, 0, 0, 0, 0, true);
        GL11.glPopMatrix();
    }

    @Override
    public void render(TileEntityBrewingStand tileEntity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        GL11.glPushMatrix();
