...
public class GuiFixedBrewingStandEF extends GuiContainer {
	...
	public GuiFixedBrewingStandEF(InventoryPlayer player, TileEntityFixedBrewingStandEF tile) {
		super(new ContainerFixedBrewingStandEF(player, tile));
	...
	protected void drawGuiContainerBackgroundLayer(float size, int x, int y) {
		...
		if (brewTime > 0) {
			...
			if (scale > 0) {
				this.drawTexturedModalRect(xZero + 63, yZero + 14 + 29 - scale, 185, 29 - scale, 12, scale);
			}
		}
		int fuel = this.tileFixedBrewingStand.getFuel();
		if (fuel > 0) {
			int sz = (int) (18.0F * fuel / this.tileFixedBrewingStand.getCurrentFuel());
			this.drawTexturedModalRect(xZero + 60, yZero + 44, 176, 29, sz, 4);
		}
	}
}
